...
public class MavLink {
    ...
    public static final int MSG_ID_INFO_EKF_STATUS_REPORT = 193;
    ...
    public static abstract class Message {
        ...
        public static Message decodeMessage(byte[] bytes) {
            ...
            switch (messageId) {
                ...
                case MSG_ID_INFO_EKF_STATUS_REPORT:
                    return new MSG_INFO_EKF_STATUS_REPORT(bytes);
                default:
                    bytes[5] = (byte) (-1 & 0xFF);
                    return new MSG_ERROR_EMPTY(bytes);
    ...
    public static class MSG_CHANGE_OPERATOR_CONTROL extends Message {
        ...
        private char passkey[]; // Password / Key, depending on version plaintext or encrypted. 25 or less characters, NULL terminated. The characters may involve A-Z, a-z, 0-9, and "!?,.-"
    ...
    public static class MSG_STATUSTEXT extends Message {
        ...
        private char text[]; // Status text message, without null termination character
    ...
    public static class MSG_PARAM_REQUEST_READ extends Message {
        ...
        private int target_system; // System ID
        private int target_component; // Component ID
        private char param_id[]; // Onboard parameter id, terminated by NULL if the length is less than 16 human-readable chars and WITHOUT null termination (NULL) byte if the length is exactly 16 chars - applications have to provide 16+1 bytes storage if the ID is stored as string
    ...
    public static class MSG_NAMED_VALUE_INT extends Message {
        ...
        private char name[]; // Name of the debug variable
    ...
    public static class MSG_INFO_EKF_STATUS_REPORT extends Message{
        ...
        protected MSG_INFO_EKF_STATUS_REPORT(byte[] bytes) {
            super(bytes);
        }

        protected MSG_INFO_EKF_STATUS_REPORT(short systemId, short componentId) {
            super(systemId, componentId);
        }
        ...
        @Override
        protected ByteBuffer decodePayload(ByteBuffer buffer) {
            velocity_variance = buffer.getFloat();
            pos_horiz_variance = buffer.getFloat();
            pos_vert_variance = buffer.getFloat();
            compass_variance = buffer.getFloat();
            terrain_alt_variance = buffer.getFloat();
            flags = buffer.getShort() & 0xffff; //uint16_t
            return buffer;
        }
        ...
        @Override
        protected ByteBuffer encodePayload(ByteBuffer buffer) {
            buffer.putFloat(velocity_variance);
            buffer.putFloat(pos_horiz_variance);
            buffer.putFloat(pos_vert_variance);
            buffer.putFloat(compass_variance);
            buffer.putFloat(terrain_alt_variance);
        ...
        @Override
        public String toString() {
            return "MSG_INFO_EKF_STATUS_REPORT {" +
    ...
    public static class MSG_ERROR_EMPTY extends Message{

        protected MSG_ERROR_EMPTY(byte[] bytes) {
            super(bytes);
        }
